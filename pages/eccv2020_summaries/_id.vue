        ...
        <div class="article-footer">
          <ul class="article-tag-list">
            <li v-for="(tag, idx) in summary.tags" :key="idx" class="article-tag-list-item">
              <nuxt-link :to="`/eccv2020_summaries/tag/${normalizeTag(tag)}`" class="article-tag-list-link">
                {{ tag }}
              </nuxt-link>
            </li>
          </ul>
...
export default {
  ...
  async asyncData({ params }) {
    let id = params.id;
    let { content: summary, meta: { totalCount } } = require(`~/static/data/eccv2020_summaries/id/${id}.json`);
