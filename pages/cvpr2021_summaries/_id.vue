    ...
    <b-container v-if="!isLoading">
      ...
      <div class="article-inner">
        ...
        <div class="article-entry">
          <b-row>
            <b-col sm="12" md="7">
              <h3 class="section-header"><b>どんな論文か？</b></h3>
        ...
        <div class="article-footer">
          <ul class="article-tag-list">
            <li v-for="(tag, idx) in summary.tags" :key="idx" class="article-tag-list-item">
              <nuxt-link :to="`/cvpr2021_summaries/tag/${normalizeTag(tag)}`" class="article-tag-list-link">
                {{ tag }}
              </nuxt-link>
            </li>
          </ul>
...
export default {
  ...
  async asyncData({ params }) {
    let id = params.id;
    let { content: summary, meta: { totalCount } } = require(`~/static/data/cvpr2021_summaries/id/${id}.json`);
  ...
  methods: {
    handleChange(page) {
      this.$router.push(`/cvpr2021_summaries/${page}`)
    },
    normalizeTag(tag) {
